...
def base64_decode(encoded_string):
    ...
    return bit_string.zfill(len(byte_array) * 8)
...
def write_file(message):
    '''密文将写入text.txt文件中'''
    try:
        fp = open('ciphertext.txt', 'w', encoding='utf-8')
        fp.write(message)
        fp.close()
    except:
...
def process_key2(key):
    '''
    key : 输入的密钥字符串
    return : 64bit 01 序列密钥 直接一个字符补8位
    '''
    bin_key = str_bit(key)
    ans = len(bin_key)
    if ans < 64:
...
def start():
    print("\nEncrypt press 1   Decrypt press 2   Exit press 3 : ", end='')
    t = input()
    if t == '1':
        ...
        write_file(base64_encode(result))
        print("Ciphertext:  " + base64_encode(result))

    elif t == '2':
        message = 'ciphertext.txt'
        message = read_file(message)
        print("Ciphertext:  " + message)
        message = base64_decode(message)
        message = bit_str(message)
